...
read -p "Have you created var GITHUB_CLIENT_ID GITHUB_CLIENT_SECRET : (yes/no) " response

if [[ "$response" == "yes" || "$response" == "y" || "$response" == "YES" ]]; then
  echo "InelObservPlatform Evironment!!!"
  ...
  echo "InelObservPlatform Evironment is Setted Up Successfully"
    ...
    echo "Kindly first create var for grafana "
    exit 1
fi
